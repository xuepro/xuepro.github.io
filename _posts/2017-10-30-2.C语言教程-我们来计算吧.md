...
```
说穿了，变量就是内存块的名字。C语言中变量都必须说明其数据类型，因为只有知道数据类型，编译器才知道给这个变量分配多大的内存，比如int整数类型变量占4个字节内存，而float类型变量占8个字节内存。变量的类型同时也规定了对这种类型的变量能进行什么样的运算。比如不能用“**取模运算%**”去得到2个实数的相除的余数。
